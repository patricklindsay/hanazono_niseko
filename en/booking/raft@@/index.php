...
if (isset($_POST['hiragana'])) {
	foreach ($_POST['hiragana'] as $v) {
		...
		if (!empty($_POST[$v])) {
			$_POST[$v] = mb_convert_kana($_POST[$v], 'cHV');
...
if (isset($_POST['zenkaku_katakana'])) {
	foreach ($_POST['zenkaku_katakana'] as $v) {
		...
		if (!empty($_POST[$v])) {
			$_POST[$v] = mb_convert_kana($_POST[$v], 'CKV');
			$_POST[$v] = delete_blank($_POST[$v]);
...
if (isset($_POST['match'])) {                                                                                                                                                   
	foreach ($_POST['match'] as $v) {
		...
		*/
		if ($_POST[$array[0]] != $_POST[$array[1]]) {
...
if (isset($_POST['either'])) {                                                                                                                                                   
	foreach ($_POST['either'] as $v) {
		$array = preg_split('/\s|,/', $v);
		$tmpl->set("either.$array[0]", false);
		if (empty($_POST[$array[0]]) && empty($_POST[$array[1]])) {
				$error_either = str_replace('{条件1}', $array[0], ERROR_EITHER);
				$error_either = str_replace('{条件2}', $array[1], $error_either);
				$tmpl->set("either.$array[0]", h($error_either));
				$global_error[] = h($error_either);
				$global_error_flag = true;
		}
	}
}
...
if (empty($page)) {
	...
	$tmpl->set('datetime', date("Y/m/d H:i:s"));
