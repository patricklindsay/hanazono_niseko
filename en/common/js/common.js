...
function langLink()
{
	...
	// URLの言語を変更する。
	en.attr('href', href);
	href = href.replace('/en', '/ja');
	jp.attr('href', href);
	
	// 英語ページしかない例外の設定。
	if (href.match(/adult-demo-equipment\.html/)) jp.attr('href', href.replace('adult-demo-equipment.html', ''));
	if (href.match(/adult-midrange-equipment\.html/)) jp.attr('href', href.replace('adult-midrange-equipment.html', ''));
	if (href.match(/clothing\.html/)) jp.attr('href', href.replace('clothing.html', ''));
	if (href.match(/kids-equipment\.html/)) jp.attr('href', href.replace('kids-equipment.html', ''));
}
...
function createFontSizeMenu()
{
	...
	ul.append($('<li id="font_size" class="png"><span class="reader">Text Size</span></li>'));
	var a = $('<a href="#TOP" class="png"><span class="reader"></span></a>').css('cursor','pointer');
	ul.append($('<li class="font_size_m btn"></li>').append(a.clone().find('span').text('Normal').end().clickpress(function(){ return setFontSize('1.2em'); })));
	ul.append($('<li class="font_size_l btn"></li>').append(a.clone().find('span').text('Large').end().clickpress(function(){ return setFontSize('1.5em'); })));
	$('#header').append(ul);
}
...
function createSearchForm()
{
	var div = '<div id="navi_search">'
			...
			+ '<input type="hidden" name="cx" value="003080879582333711453:gklfodhnsaq" />'
