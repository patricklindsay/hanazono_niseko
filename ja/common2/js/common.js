...
$(function(){
	...
	*/
	...
	$('#nav a.lv1').each(function(){
		var a = $(this);
		a.click(function(){
			var id = a.parent('li').attr('id');
			if (id.has(/nav_snow|nav_green|nav_summer|nav_winter|nav_english|nav_japanese|nav_educational_travel/)) return true;
			/*
			if (a.parent('li').attr('id') == 'nav_snow') return true;
			if (a.parent('li').attr('id') == 'nav_summer') return true;
			if (a.parent('li').attr('id') == 'nav_english') return true;
			*/
			
			if (a.attr('href').has('nav_packages_deals')) {
				location.href = 'http://vacationniseko.com/ja/';
				return false;
			}
	...
	// 英語リンクを対応するURLに変更。
	$('#nav_english a').attr('href', location.href.replace('/ja', '/en'));
...
function setBookingForm(){
	...
	bf.load(url + '?' + Date.now(), function(){
		...
		$('#booking_form #serchbtn').click(function(){
			var ci = Date.create($('#checkin').val(), 'ja');
			ci = ci.format('{yy}{MM}{dd}', 'ja');
			$('#ci').val(ci);
			return true;
		});
